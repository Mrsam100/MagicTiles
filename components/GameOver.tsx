...
const GameOver: React.FC<Props> = ({ username, scoreData, review, onRestart, onMenu }) => {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-start bg-white z-[100] px-6 pt-10 overflow-y-auto custom-scrollbar pb-10">
      ...
      <div className="w-full max-w-sm flex flex-col items-center relative z-10">
        ...
        <div className="relative mb-6 group">
          ...
          <div className={`w-20 h-20 rounded-full flex items-center justify-center border-8 shadow-[0_20px_50px_rgba(0,0,0,0.1)] relative transition-all duration-700 animate-float-slow ${isNewRecord ? 'bg-[#0f1c3a] border-[#1e293b]' : 'bg-slate-50 border-red-50/50'}`}>
            <i className={`fa-solid ${isNewRecord ? 'fa-crown text-white' : 'fa-triangle-exclamation text-red-500'} text-3xl drop-shadow-lg`}></i>
        ...
        <div className={`w-full mb-5 transform transition-all duration-700 ${showStats ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-10 opacity-0 scale-95'}`}>
           <div className="bg-[#0f1c3a] p-6 rounded-[2rem] text-center shadow-[0_30px_60px_-15px_rgba(15,28,58,0.25)] relative overflow-hidden group">
