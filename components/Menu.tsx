  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-start bg-white px-5 py-8 overflow-hidden animate-simple-fade">
      ...
      <div className="relative z-10 w-full flex flex-col items-center mb-6 text-[#0f1c3a]">
        ...
        <p className="text-[10px] text-slate-600 font-orbitron tracking-[0.4em] uppercase font-black mt-1.5">Sourav Rajput Edition</p>
      </div>
      ...
      <div className="relative z-10 w-full mb-5">
        <div className="bg-slate-50 w-full py-3 px-5 rounded-[1.5rem] border border-slate-100 flex justify-between items-center shadow-sm relative overflow-hidden group">
          <div className="flex flex-col">
            <span className="text-[10px] font-orbitron text-slate-600 font-black uppercase tracking-widest mb-0.5 opacity-70">BEST SCORE</span>
          ...
          {SONGS.map((song) => (
            ...
            >
              ...
              <div className="mt-auto overflow-hidden w-full text-left">
                <span className={`block text-[11px] font-orbitron font-black leading-tight uppercase truncate mb-0.5 ${selectedSong.id === song.id ? 'text-white' : 'text-[#0f1c3a]'}`}>
                  {song.name}
                </span>
                <span className={`block text-[10px] font-orbitron font-black uppercase tracking-widest truncate ${selectedSong.id === song.id ? 'text-slate-300' : 'text-slate-500'}`}>
                  {song.genre}
                </span>
              </div>
            </button>
